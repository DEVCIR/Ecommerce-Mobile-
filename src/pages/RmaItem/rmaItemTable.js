...
const RmaItemTable = (props) => {
    ...
    const fetchRmaData = async (page = 1) => {
        try {
            setLoading(true);
            const response = await fetch(`${BASE_URL}/rma-items?page=${page}`);
            ...
            if (result.data && Array.isArray(result.data.data)) {
                ...
                console.log("RMA Items loaded:", result.data.data);
            } else {
                setRmaData([]);
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await fetch(`${BASE_URL}/rma-items/${rmaToDelete.id}`, {
                method: 'DELETE',
                headers: {
                    ...
                    <Card>
                        <CardBody>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                <CardTitle className="h4 mb-0">Return Merchandise Authorizations Items</CardTitle>
                                <Button 
                                    color="primary" 
                                    onClick={props.onAddRmasClick}
                                >
                                    <i className="bx bx-plus me-1"></i> Add RMA Item
                                </Button>
                            </div>
                            
                            {loading ? (
                                <div className="text-center">Loading RMA Itemss...</div>
                            ) : (
                                    ...
                                    <Table className="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            {rmaData.length > 0 ? (
                                                rmaData.map(rma => (
                                                    <tr key={rma.id}>
                                                        <td>{displayField(rma.rma?.rma_number)}</td>
                                                        <td>{displayField(rma.inventory.variant.product?.model_name)}</td>
                                                        <td>{displayField(rma.rma.customer?.user?.name)}</td>
                                                        <td>{displayField(rma.rma.order?.order_number)}</td>
                                                        <td>{formatDate(rma.rma?.request_date)}</td>
...
export default connect(null, { setBreadcrumbItems })(RmaItemTable);
