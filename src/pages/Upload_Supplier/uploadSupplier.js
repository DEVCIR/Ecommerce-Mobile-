...
const UploadSupplier = (props, setViewToTable) => {
  ...
  const fetchSupplierData = async (supplier_name, supplier_email) => {
    try {
      const response = await fetch(`http://localhost:8000/api/users?name=${supplier_name}&email=${supplier_email}`);
  ...
  const handlePrintNamesAndEmails = async () => {
    ...
    for (let i = 0; i < userNames.length; i++) {
      ...
      try {
        ...
        try {
          const supplierResponse = await fetch('http://localhost:8000/api/suppliers', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
